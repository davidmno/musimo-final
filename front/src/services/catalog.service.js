...
function writeSession(key, value, ttl) {
  try {
    sessionStorage.setItem(`${SESSION_PREFIX}${key}`, JSON.stringify({ value, expiresAt: Date.now() + ttl }));
  } catch {
...
function cachedRequest(key, loader, ttl, { share = true } = {}) {
  ...
  if (memoryHit?.expiresAt > Date.now()) return Promise.resolve(memoryHit.value);
...
export function getRelease(id) {
  return cachedRequest(`release:${id}`, () => apiRequest(`/catalog/releases/${encodeURIComponent(id)}`), 60 * MINUTE);
}

export function getReleaseTracks(id) {
  return cachedRequest(`tracks:v2:${id}`, () => apiRequest(`/catalog/releases/${encodeURIComponent(id)}/tracks`), 24 * 60 * MINUTE);
}

export function getArtist(id) {
  return cachedRequest(`artist:${id}`, () => apiRequest(`/catalog/artists/${encodeURIComponent(id)}`), 60 * MINUTE);
}

export function getArtistReleases(id, limit = 30) {
  return cachedRequest(
    `artist-releases:${id}:${limit}`,
    () => apiRequest(`/catalog/artists/${encodeURIComponent(id)}/releases?limit=${limit}`),
    30 * MINUTE,
  );
}

export function getNewReleases(limit = 12) {
  return cachedRequest(`new-releases:${limit}`, () => apiRequest(`/catalog/new-releases?limit=${limit}`), 30 * MINUTE);
}
